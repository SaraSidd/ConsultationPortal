...
class DbOperation
{
    ...
    public function addMessage($id,$reciever_id,$message){
        $stmt = $this->conn->prepare("INSERT INTO messages (message,users_id,reciever_id) VALUES (?,?,?)");
        $stmt->bind_param("si",$message,$id,"4");
        if($stmt->execute())
            return true;
        return false;
    }
    ...
    public function getMessages(){
        $stmt = $this->conn->prepare("SELECT a.id, a.message, a.sentat,  a.users_id, b.name FROM messages a, users b WHERE a.users_id = b.id and reciever_id = 4 ORDER BY a.id ASC;");
        $stmt->execute();
        $result = $stmt->get_result();
        return $result;
    }
